...
# 每份材料冻结的 BM25 证据 chunk 上限（FileLocator.chunk_ids ≤32 的子集）
SOURCE_CHUNKS_PER_FILE = 8

# 图片
IMAGE_SEARCH_BUDGET = 8
IMAGE_COUNT_MAX = 12
IMAGE_BYTES_MAX_DOWNLOAD = 8 * 1024 * 1024   # 单张下载上限（§8.3.4）
...
# 图库限流（§8.2：程序读取实际响应头更新；这里只是初始配置上限）
PEXELS_HOURLY_LIMIT_DEFAULT = 200
PEXELS_MONTHLY_LIMIT_DEFAULT = 20000
PIXABAY_PER_MINUTE_LIMIT = 100
PIXABAY_QUERY_MAX_CHARS = 100
# 图片 CDN 白名单（§8.3：API 主机与下载主机分开；模型不得自动扩域）
