...
- [ ] A02 实现课堂 root、ID 校验、文件锁/CAS、atomic_write_bytes、lesson/job/run 读写、manifest hash 和可重建 index。
- [ ] A03 为新增 root 同时修改 storage_sandbox、orphan_cleanup、account_data/trash 的入口和 tombstone 防晚写逻辑；添加 fixtures 用已隔离临时目录。
...
- [ ] C01 将原工作区材料解析封装为 classroom.sources，支持教材组卷顺序、章节、source hash 和显式 session attachments。
- [ ] C02 实现 Tavily search/extract adapter、结果部分失败、时效元数据、预算与缓存。
...
- [ ] D05 实现单页重生成/手工修改/换主题/换图/刷新来源五种 revision operation，保留旧版本可用。
- [ ] D06 实现 job snapshot SSE 和轮询 fallback；前端断线不影响 worker。
...
- [ ] E01 增路由和 WorkspaceModeBar、工作区菜单/固定课堂入口、Sidebar 批量摘要。
- [ ] E02 列表三态、Pager、CreateLessonModal、模板预览、默认值和能力不足提示。
- [ ] E03 生成阶段、关闭后恢复、取消/重试、可选大纲审核、失败草稿预览。
...
- [ ] G01 实现 reducer、audio controller、共享 audio focus、media clock、epoch 丢弃机制。
- [ ] G02 将段动作绑定 frame MessagePort；实现字幕、目录、页/段切换、语速/音量、阅读模式。
- [ ] G03 实现 run 创建/恢复、进度 CAS/节流、lease 心跳/接管、refresh/background/offline 行为。
- [ ] G04 实现专注/全屏/键盘/触控/reduced-motion；播放器之外不挂全局快捷键。
